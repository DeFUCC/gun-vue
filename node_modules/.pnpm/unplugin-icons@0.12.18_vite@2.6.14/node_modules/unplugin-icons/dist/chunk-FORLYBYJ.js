...
var _chunkIJYRCWEXjs = require('./chunk-IJYRCWEX.js');
...
var debug2 = _debug2.default.call(void 0, "unplugin-icons:icon");
var debugModern = _debug2.default.call(void 0, "unplugin-icons:modern");
var debugLegacy = _debug2.default.call(void 0, "unplugin-icons:legacy");
var _collections = {};
...
async function loadCollection(name, autoInstall = false) {
  ...
  async function task() {
    ...
    if (!jsonPath && !isLegacyExists && autoInstall) {
      await _chunkWOL7QKXNjs.tryInstallPkg.call(void 0, `@iconify-json/${name}`);
      jsonPath = _localpkg.resolveModule.call(void 0, `@iconify-json/${name}/icons.json`);
    }
    if (jsonPath) {
      return JSON.parse(await _fs.promises.readFile(jsonPath, "utf8"));
    } else {
...
function searchForIcon(iconSet, collection, ids, options) {
  ...
  let iconData;
  for (const id of ids) {
    iconData = _geticon.getIconData.call(void 0, iconSet, id, true);
    if (iconData) {
      debug2(`${collection}:${id}`);
      const scale = (_a = options == null ? void 0 : options.scale) != null ? _a : 1;
      const { attributes, body } = _build.iconToSVG.call(void 0, iconData, _chunkIJYRCWEXjs.__spreadProps.call(void 0, _chunkIJYRCWEXjs.__spreadValues.call(void 0, {}, _customisations.defaults), {
        height: `${scale}em`,
        width: `${scale}em`
      }));
...
var JSXCompiler = async (svg, collection, icon, options) => {
  const svgr = (await _localpkg.importModule.call(void 0, "@svgr/core")).default;
  let res = await svgr(svg, {}, { componentName: _chunkWOL7QKXNjs.camelize.call(void 0, `${collection}-${icon}`) });
...
var Vue2Compiler = async (svg, collection, icon) => {
  const { compile } = await Promise.resolve().then(() => _chunkIJYRCWEXjs.__toModule.call(void 0, _chunkIJYRCWEXjs.__require.call(void 0, "vue-template-compiler")));
  const transpile = (await Promise.resolve().then(() => _chunkIJYRCWEXjs.__toModule.call(void 0, _chunkIJYRCWEXjs.__require.call(void 0, "vue-template-es2015-compiler")))).default;
...
var Vue3Compiler = async (svg, collection, icon) => {
  const { compileTemplate } = await Promise.resolve().then(() => _chunkIJYRCWEXjs.__toModule.call(void 0, _chunkIJYRCWEXjs.__require.call(void 0, "@vue/compiler-sfc")));
...
var WebComponentsCompiler = (svg, collection, icon, { webComponents: options }) => {
  ...
  const name = _chunkWOL7QKXNjs.camelize.call(void 0, id);
  return `
...
var debug3 = _debug2.default.call(void 0, "unplugin-icons:custom");
...
// src/core/loader.ts
var debug4 = _debug2.default.call(void 0, "unplugin-icons:load");
...
var legacyExists = _localpkg.isPackageExists.call(void 0, "@iconify/json");
async function getBuiltinIcon(collection, icon, options, warn = true) {
  ...
  if (warn)
    _chunkWOL7QKXNjs.warnOnce.call(void 0, `failed to load \`@iconify-json/${collection}\`, have you installed it?`);
  return null;
}
