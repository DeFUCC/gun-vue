...
var parseColorUtil = (body, theme) => {
  ...
  if (!name)
    return;
  let color;
  const bracket = _chunkSCNDCSE5js.handler.bracket(main) || main;
...
var colorResolver = (attribute, varName) => ([, body], { theme }) => {
  ...
  if (!color)
    return;
  if (rgba) {
    const a = opacity2 ? opacity2[0] === "[" ? _chunkSCNDCSE5js.handler.bracket.percent(opacity2) : parseFloat(opacity2) / 100 : rgba[3];
...
var opacity = [
  [/^op(?:acity)?-?(.+)$/, ([, d]) => ({ opacity: _chunkSCNDCSE5js.handler.bracket.percent.cssvar(d) })]
];
var textColors = [
  [/^(?:text|color|c)-(.+)$/, colorResolver("color", "text")],
  [/^(?:text|color|c)-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-text-opacity": _chunkSCNDCSE5js.handler.bracket.percent.cssvar(opacity2) })]
];
var textDecorationColors = [
  ...
  [/^underline-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-line-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var textStrokeColors = [
  [/^text-stroke-(.+)$/, colorResolver("-webkit-text-stroke-color", "text-stroke")],
  [/^text-stroke-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-text-stroke-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var bgColors = [
  [/^bg-(.+)$/, colorResolver("background-color", "bg")],
  [/^bg-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-bg-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var borderColors = [
  [/^(?:border|b)-(.+)$/, colorResolver("border-color", "border")],
  [/^(?:border|b)-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-border-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var ringColors = [
  [/^ring-(.+)$/, colorResolver("--un-ring-color", "ring")],
  [/^ring-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-ring-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var ringOffsetColors = [
  [/^ring-offset-(.+)$/, colorResolver("--un-ring-offset-color", "ring-offset")],
  [/^ring-offset-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-ring-offset-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var divideColors = [
  [/^divide-(.+)$/, colorResolver("border-color", "divide")],
  [/^divide-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-divide-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var fillColors = [
  ["fill-none", { fill: "none" }],
  [/^fill-(.+)$/, colorResolver("fill", "fill")],
  [/^fill-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-fill-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
...
function handlerBorder([, a, b]) {
  const [d, s = "1"] = _chunkSCNDCSE5js.directionMap[a] ? [a, b] : ["", a];
  const v = _chunkSCNDCSE5js.handler.bracket.px(s);
  if (v != null) {
    return [
      ..._chunkSCNDCSE5js.directionMap[d].map((i) => [`border${i}-width`, v]),
      ["border-style", "solid"]
    ];
  }
}
function handlerRounded([, a, b], { theme }) {
  var _a;
  const [d, s = "DEFAULT"] = _chunkSCNDCSE5js.cornerMap[a] ? [a, b] : ["", a];
  const v = ((_a = theme.borderRadius) == null ? void 0 : _a[s]) || _chunkSCNDCSE5js.handler.bracket.fraction.rem(s);
  if (v != null)
    return _chunkSCNDCSE5js.cornerMap[d].map((i) => [`border${i}-radius`, v]);
}
...
var bgGradients = [
  ...
  [/^from-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-from-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })],
  [/^to-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-to-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })],
  [/^via-op(?:acity)?-?(.+)$/m, ([, opacity2]) => ({ "--un-via-opacity": _chunkSCNDCSE5js.handler.bracket.percent(opacity2) })]
];
var bgImages = [
...
var percentWithDefault = (defaultValue = "1") => (str) => {
  const v = str ? _chunkSCNDCSE5js.handler.bracket.percent(str) : defaultValue;
...
var filters = [
  ...
  [/^(backdrop-)?blur(?:-(.+))?$/, toFilter("blur", (s, theme) => {
    var _a;
    return ((_a = theme.blur) == null ? void 0 : _a[s || "DEFAULT"]) || _chunkSCNDCSE5js.handler.bracket.px(s);
  })],
  [/^(backdrop-)?brightness-(\d+)$/, toFilter("brightness", (s) => _chunkSCNDCSE5js.handler.bracket.percent(s))],
  [/^(backdrop-)?contrast-(\d+)$/, toFilter("contrast", (s) => _chunkSCNDCSE5js.handler.bracket.percent(s))],
  [/^()?drop-shadow(?:-(.+))?$/, toFilter("drop-shadow", (s, theme) => {
    var _a;
    const v = _chunkSCNDCSE5js.handler.bracket(s) || ((_a = theme.dropShadow) == null ? void 0 : _a[s || "DEFAULT"]);
    if (v)
      return _core.toArray.call(void 0, v).map((v2) => `drop-shadow(${v2})`).join(" ");
  })],
  [/^(backdrop-)?grayscale(?:-(\d+))?$/, toFilter("grayscale", percentWithDefault())],
  [/^(backdrop-)?hue-rotate-(\d+)$/, toFilter("hue-rotate", (s) => `${_chunkSCNDCSE5js.handler.bracket.number(s)}deg`)],
...
var fontSizes = [
  [/^text-([^-]+)$/, ([, s = "base"], { theme }) => {
    var _a;
    const result = _core.toArray.call(void 0, ((_a = theme.fontSize) == null ? void 0 : _a[s]) || _chunkSCNDCSE5js.handler.bracket.rem(s));
...
var fontWeights = [
  [/^(?:font|fw)-?([^-]+)$/, ([, s]) => {
    const v = weightMap[s] || _chunkSCNDCSE5js.handler.number(s);
    if (v)
...
var leadings = [
  [/^(?:leading|lh)-([^-]+)$/, ([, s], { theme }) => {
    var _a;
    const v = ((_a = theme.lineHeight) == null ? void 0 : _a[s]) || _chunkSCNDCSE5js.handler.bracket.rem(s);
    if (v !== null)
...
var trackings = [
  [/^tracking-([^-]+)$/, ([, s], { theme }) => {
    var _a;
    const v = ((_a = theme.letterSpacing) == null ? void 0 : _a[s]) || _chunkSCNDCSE5js.handler.bracket.rem(s);
    if (v !== null)
...
var tabSizes = [
  [/^tab-?([^-]*)$/, ([, s]) => {
    s = s || "4";
    const v = _chunkSCNDCSE5js.handler.bracket.global.number(s);
    if (v !== null) {
      return {
        "-moz-tab-size": v,
        "-o-tab-size": v,
...
var textDecorationLengths = [
  [/^underline-([^-]+)$/, ([, s]) => {
    const v = s === "auto" ? s : _chunkSCNDCSE5js.handler.bracket.px(s);
    if (v != null)
...
var textIndents = [
  [/^indent(?:-(.+))?$/, ([, s], { theme }) => {
    var _a;
    const v = ((_a = theme.textIndent) == null ? void 0 : _a[s || "DEFAULT"]) || _chunkSCNDCSE5js.handler.bracket.cssvar.fraction.rem(s);
    if (v != null)
...
var textStrokeWidths = [
  [/^text-stroke(?:-(.+))?$/, ([, s], { theme }) => {
    var _a;
    const v = ((_a = theme.textStrokeWidth) == null ? void 0 : _a[s || "DEFAULT"]) || _chunkSCNDCSE5js.handler.bracket.cssvar.px(s);
    if (v != null)
...
var textShadows = [
  [/^text-shadow(?:-(.+))?$/, ([, s], { theme }) => {
    var _a;
    const v = ((_a = theme.textShadow) == null ? void 0 : _a[s || "DEFAULT"]) || _chunkSCNDCSE5js.handler.bracket.cssvar(s);
    if (v != null)
...
var gaps = [
  [/^(?:flex-|grid-)?gap-([^-]+)$/, ([, s]) => {
    const v = _chunkSCNDCSE5js.handler.bracket.rem(s);
    if (v != null) {
      return {
        "grid-gap": v,
        "gap": v
...
var calSize = (s, theme) => {
  var _a;
  return _core.toArray.call(void 0, ((_a = theme.fontSize) == null ? void 0 : _a[s]) || _chunkSCNDCSE5js.handler.bracket.rem(s))[0];
};
...
var grids = [
  ...
  [/^(?:grid-)?(row|col)-(.+)$/, ([, d, v]) => {
    ...
    let raw = _chunkSCNDCSE5js.handler.bracket(v);
    if (raw)
      return { [key]: raw };
    const parts = v.split("-");
    ...
    if (parts[0] === "span") {
      if (parts[1] === "full")
        return { [key]: "1/-1" };
      raw = (_a = _chunkSCNDCSE5js.handler.number.bracket(parts[1])) == null ? void 0 : _a.toString().replace(/_/g, " ");
      if (raw)
...
var orders = [
  [/^order-(.+)$/, ([, v]) => ({ order: { first: "-9999", last: "9999", none: "0" }[v] || _chunkSCNDCSE5js.handler.bracket.number(v) })]
];
...
function handleInsetValue(v) {
  var _a;
  return (_a = { auto: "auto", full: "100%" }[v]) != null ? _a : _chunkSCNDCSE5js.handler.bracket.fraction.cssvar.rem(v);
}
var insets = [
  [/^(top|left|right|bottom|inset)-(.+)$/, ([, d, v]) => ({ [d]: handleInsetValue(v) })],
  [/^inset-([xy])-(.+)$/, ([, d, v]) => {
    const r = handleInsetValue(v);
    if (r != null && d in _chunkSCNDCSE5js.directionMap)
      return _chunkSCNDCSE5js.directionMap[d].map((i) => [i.slice(1), r]);
  }]
];
var floats = [
...
var zIndexes = [
  ["z-auto", { "z-index": "auto" }],
  [/^z-([^-]+)$/, ([, v]) => ({ "z-index": _chunkSCNDCSE5js.handler.number(v) })]
];
var objectPositions = [
...
var rings = [
  [/^ring-?(.*)$/, ([, d]) => {
    const value = _chunkSCNDCSE5js.handler.px(d || "1");
    if (value) {
      return {
...
function getThemeValue(minmax, hw, theme, prop) {
  ...
  if (minmax)
    str = `${minmax}${_chunkSCNDCSE5js.capitalize.call(void 0, str)}`;
  return (_a = theme[str]) == null ? void 0 : _a[prop];
}
var sizes = [
  [/^(?:(min|max)-)?(w|h)-(.+)$/, ([, m, w, s], { theme }) => {
    const v = getThemeValue(m, w, theme, s) || _chunkSCNDCSE5js.handler.bracket.cssvar.fraction.rem(s);
    if (v != null)
...
var aspectRatio = [
  ...
  [/^aspect-ratio-(.+)$/, ([, d]) => {
    const v = (/^\d+\/\d+$/.test(d) ? d : null) || _chunkSCNDCSE5js.handler.bracket.cssvar.number(d);
    if (v != null)
...
var directionSize = (prefix) => ([_, direction, size]) => {
  const v = _chunkSCNDCSE5js.handler.bracket.rem.fraction.cssvar(size);
  if (v)
    return _chunkSCNDCSE5js.directionMap[direction].map((i) => [prefix + i, v]);
};
var paddings = [
...
function handleTranslate([, d, b]) {
  const v = _chunkSCNDCSE5js.handler.bracket.fraction.rem(b);
  if (v != null) {
    return [
      ..._chunkSCNDCSE5js.xyzMap[d].map((i) => [`--un-translate${i}`, v])
    ];
  }
}
function handleScale([, d, b]) {
  const v = _chunkSCNDCSE5js.handler.bracket.fraction.percent(b);
  if (v != null) {
    return [
      ..._chunkSCNDCSE5js.xyzMap[d].map((i) => [`--un-scale${i}`, v])
    ];
  }
}
function handleRotate([, b]) {
  const v = _chunkSCNDCSE5js.handler.bracket.number(b);
  if (v != null)
...
var parseOutlineSize = (s) => {
  ...
  const size = _chunkSCNDCSE5js.handler.bracket.fraction.rem(s.replace(/^(offset\-|width\-)/, ""));
  if (size) {
    return {
...
var caretOpacity = [
  [/^caret-op(?:acity)?-?(.+)$/, ([, d]) => ({ "--un-caret-opacity": _chunkSCNDCSE5js.handler.bracket.percent(d) })]
];
var caretColors = [
...
var placeholder = [
  [
    ...
    ([, d]) => ({
      "placeholder-opacity": _chunkSCNDCSE5js.handler.bracket.percent(d)
    })
  ],
  [
    /^placeholder-(?!opacity)(.+)$/,
...
${constructCSS({ animation: `${name} 1s linear infinite` })}`;
  ...
  [/^animate(?:-duration)?-((.+)(?:(s|ms)?))$/, ([, d]) => ({ "animation-duration": _chunkSCNDCSE5js.handler.bracket.time(d.replace(/-duration/, "")) })],
  [/^animate-delay-((.+)(?:(s|ms)?))$/, ([, d]) => ({ "animation-delay": _chunkSCNDCSE5js.handler.bracket.time(d) })],
...
function handlerDivide([, a, b]) {
  ...
  const v = _chunkSCNDCSE5js.handler.bracket.px(s);
  if (v != null) {
    const results = _chunkSCNDCSE5js.directionMap[d].map((item) => {
