...
<template lang="pug">
button.button.absolute.z-200.top-4.left-4(
  v-if="(!panelOpen && !isLarge)" 
  @click="panelOpen = true"
  ) {{ title }}
transition(name="fade")
  .px-1.py-2.flex.flex-col.bg-dark-50.bg-opacity-95.gap-2.min-h-full.overflow-y-scroll.scroll-smooth.absolute.sm-static.z-20.w-220px.max-w-full.max-h-full.text-light-900.backdrop-filter.backdrop-blur-xl(
    ...
    .flex.flex-wrap
      ...
      .flex-1
      .cursor-pointer.self-center.text-2xl.p-2(
        @click="adding = !adding"
        )
        transition(
          name="fade" 
