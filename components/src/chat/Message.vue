...
import { selectedUser, useUser } from '..';
...
.p-1.flex.flex-col.w-full.gap-1(:style="{alignItems:isMe ? 'end': 'start'}")
  ...
  .px-2.py-1.bg-light-300.bg-opacity-80.rounded-b-xl.max-w-max.break-all.overflow-hidden(:style="{borderTopLeftRadius: isMe? '12px': '0px', borderTopRightRadius: isMe ? '0px': '12px'}")
    slot {{ text }}
</template>