...
form.w-full.flex.flex-col.p-2.shadow-xl.m-1.rounded-2xl.mb-6(action="javascript:void(0);")
  ...
  textarea.text-1rem.leading-relaxed(v-model="postData.statement" placeholder="Short text statement")
  .flex.flex-wrap
    button.button.m-1(
    ...
    .flex.flex-wrap
      button.button.m-1(@click="add.youtube = !add.youtube" :class="{ active: postData.youtube }")
        la-youtube

    button.m-1.button(@click="add.content = true" :class="{ active: postData.content }")
      mdi-text-long
    .flex-1
    .flex.justify-center.flex-1
      button.plus.button.flex-1.justify-center.m-1( :disabled="!hasContent" type="submit" @click="submit()")
        la-check
