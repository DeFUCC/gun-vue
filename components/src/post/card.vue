...
.card(
  ...
  .flex.flex-wrap.items-center.max-w-full.w-full.backdrop-blur-lg.rounded-md.bg-light-100.backdrop-blur-sm.backdrop-filter.shadow-md(
    style="flex: 14 1 240px"
  )
    ...
    .flex.flex-col.p-2.overflow-hidden(style="flex: 10 1 180px")
      ...
      .flex.items-center.flex-wrap.items-center.mt-2.gap-2
        ...
        slot
    .flex-1.text-xs
    .flex.gap-1.rounded-md.p-1.bg-dark-100.bg-opacity-20.flex-wrap.items-center(style="flex: 1 1 20%" v-if="actions")
...
.card {
  @apply transition duration-300ms ease-out min-w-280px p-1 rounded-lg cursor-pointer flex flex-wrap items-end bg-cover bg-center;
